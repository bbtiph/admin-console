...
import kaz.post.crmserver.entity.PaymentCardEntity;
...
public interface PaymentCardRepository extends JpaRepository<PaymentCardEntity, Long> {
	...
	@Transactional
	@Query("select count(*) from PaymentCardEntity where created_date > TO_DATE(:begData, 'YYYYMMDD') and created_date < TO_DATE(:endData, 'YYYYMMDD')")
