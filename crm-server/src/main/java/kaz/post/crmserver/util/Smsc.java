...
public class Smsc {
	...
	String SMSC_LOGIN = "maratkalibek";     // логин клиента
    String SMSC_PASSWORD = "931ddbebc7f8968ec901740aded7b54c";  // пароль или MD5-хеш пароля в нижнем регистре
    ...
    public String[] send_sms(String phones, String message, int translit, String time, String id, int format, String sender, String query) {
        ...
        if (SMSC_DEBUG) {
            if (Integer.parseInt(m[1]) > 0) {
				...
				log.info("Ошибка №" + Math.abs(Integer.parseInt(m[1])));
				log.info(Integer.parseInt(m[0]) > 0 ? (", ID: " + m[0]) : "");
			}
        }
